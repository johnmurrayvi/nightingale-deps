...
MP4::Tag::parseCovr(MP4::Atom *atom, TagLib::File *file)
{
  ...
  while(pos < data.size()) {
    ...
    // XXX RECHECK THIS
    // POTI did away with the if statement, basically reading whatever was there,
    // but the if statement previously only checked for PNG and JPEG
...
MP4::Tag::updateOffsets(long delta, unsigned long offset)
{
  ...
  if(moov) {
    ...
    for(unsigned int i = 0; i < stco.size(); i++) {
      ...
      while(count--) {
        // XXX RECHECK THIS
        unsigned long o = static_cast<long>(data.toUInt(pos));
        if(o > offset) {
          o += delta;
        }
