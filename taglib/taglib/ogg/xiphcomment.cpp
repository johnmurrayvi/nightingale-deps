...
List<TagLib::FLAC::Picture*> Ogg::XiphComment::artwork() const
{
  List<TagLib::FLAC::Picture*> artwork;
...
void Ogg::XiphComment::addArtwork(TagLib::FLAC::Picture &pic)
{
  ByteVector bv = pic.render(true);
  addField("METADATA_BLOCK_PICTURE", bv.data());
}

void Ogg::XiphComment::setArtwork(List<TagLib::FLAC::Picture*> artworkList)
{
  // first remove all artwork fields
  removeField("METADATA_BLOCK_PICTURE");

  // next add all artwork in the given list
  for (List<TagLib::FLAC::Picture*>::Iterator it = artworkList.begin();
       it != artworkList.end();
       ++it)
  {
    addArtwork(**it);
